# IAM Role for Lambda Ingestion Functions
...
# IAM Role for Glue Processing Job
resource "aws_iam_role" "glue_processing" {
  name = "${var.project_name}-glue-processing-role"
...
resource "aws_iam_role_policy" "glue_processing" {
  name = "${var.project_name}-glue-processing-policy"
  role = aws_iam_role.glue_processing.id
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          "${aws_s3_bucket.data_lake.arn}/rejected/*"
        ]
      },
      {
        Effect = "Allow"
        ...
        Resource = [
          ...
          "arn:aws:glue:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:database/${var.project_name}",
          "arn:aws:glue:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:table/${var.project_name}/*"
        ]
      },
      {
        Effect = "Allow"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "${aws_s3_bucket.data_lake.arn}/analytics/*"
      },
      {
        Effect = "Allow"
        Action = [
      ...
      },
      {
        ...
        Resource = "arn:aws:athena:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:workgroup/${var.project_name}"
      }
    ]
  })
}
